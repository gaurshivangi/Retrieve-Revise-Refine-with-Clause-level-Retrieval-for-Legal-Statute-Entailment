...
if __name__=="__main__":
    ...
    parser.add_argument("--val_check_interval", default=0.1, type=float, help="When to run validation (float fraction of epoch or int number of batches).")
    ...
    parser.add_argument("--civi_code_path", default=None, type=str, help="civil code path (optional for evaluation)")
    ...
    train_loader = DataLoader(df_train.values, batch_size=opts.batch_size, collate_fn=coliee_data_preprocessor, shuffle=True, num_workers=4, pin_memory=True)
    df_dev = pd.read_csv(f"{opts.data_dir}/dev.csv")
    dev_loader = DataLoader(df_dev.values, batch_size=opts.batch_size, collate_fn=coliee_data_preprocessor, shuffle=False, num_workers=2, pin_memory=True)
    df_test = pd.read_csv(f"{opts.data_dir}/test.csv")
    test_loader = DataLoader(df_test.values, batch_size=opts.batch_size, collate_fn=coliee_data_preprocessor, shuffle=False, num_workers=2, pin_memory=True)
    ...
    if os.path.exists(test_submit_path):
        ...
        test2_loader = DataLoader(df_test2.values, batch_size=opts.batch_size, collate_fn=coliee_data_preprocessor, shuffle=False, num_workers=2, pin_memory=True)
    else:
    ...
    # Simple callback to save pickle file when checkpoint is saved
    class PickleSaverCallback(Callback):
        def on_save_checkpoint(self, trainer, pl_module, checkpoint):
            try:
                weights_path = os.path.join(trainer.log_dir or trainer.default_root_dir, "model_weights.pkl")
                pl_module.model.eval()
                state_dict = {k: v.cpu() if isinstance(v, torch.Tensor) else v 
                             for k, v in pl_module.model.state_dict().items()}
                model_info = {
                    'state_dict': state_dict,
                    'config': pl_module.config.to_dict(),
                    'args': vars(pl_module.args),
                    'model_name': pl_module.args.model_name_or_path,
                    'current_epoch': trainer.current_epoch,
                    'global_step': trainer.global_step,
                    'num_epochs': trainer.max_epochs,
                    'batch_size': pl_module.args.batch_size,
                    'lr': pl_module.args.lr,
                    'max_seq_length': pl_module.args.max_seq_length,
                    'training_completed': False,
                    'saved_from_checkpoint': True
                }
                with open(weights_path, 'wb') as f:
                    pickle.dump(model_info, f)
                if hasattr(pl_module, 'result_logger'):
                    pl_module.result_logger.info(f"Pickle file saved to {weights_path}")
            except Exception as e:
                if hasattr(pl_module, 'result_logger'):
                    pl_module.result_logger.warning(f"Failed to save pickle: {e}")
    
    pickle_callback = PickleSaverCallback()
    
    # Determine accelerator and devices based on GPU availability
    import torch
    if torch.cuda.is_available() and len(opts.gpus) > 0 and opts.gpus[0] >= 0:
        accelerator = 'gpu'
        devices = opts.gpus
        print(f"Using GPU training on devices: {devices}")
    else:
        accelerator = 'cpu'
        devices = 1
        print("Using CPU training (GPU not available or disabled)")
    
    trainer = Trainer(max_epochs=opts.max_epochs, 
                      accelerator=accelerator,
                      devices=devices, 
                      callbacks=[checkpoint_callback, pickle_callback], 
    ...
    if not opts.no_train:
        ...
        # Save final model weights as pickle file (marking training as completed)
        weights_path = os.path.join(opts.log_dir, "model_weights.pkl")
        model.model.eval()
        state_dict = {k: v.cpu() if isinstance(v, torch.Tensor) else v 
        ...
        model_info = {
            ...
            'saved_from_checkpoint': False
        }
        with open(weights_path, 'wb') as f:
            pickle.dump(model_info, f)
        model.result_logger.info(f"Final model weights saved to {weights_path} (training completed)")
        print(f"\n{'='*80}")
    ...
    for data_loader in data_sources: 
        ...
        if os.path.exists(main_pred_file):
            ...
            def enss_procedure(main_pred_file, addition_pred_files, output_file, addition_limit=None, relevant_limit=None):
                ...
                # evaluate
                input_test = f"../COLIEE2025statute_data-English/train/riteval_{data_query_id}_en.xml"
