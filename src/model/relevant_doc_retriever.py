...
class RelevantDocClassifier(pl.LightningModule):
    ...
    def on_validation_epoch_end(self, no_log_tensorboard=False, main_prediction_enss=None):
        ...
        gr_pred, gr_gold =  aggregrate_val(validation_outputs)
        main_gr_pred = None
    ...
    def validation_epoch_end(self, predictions=None, no_log_tensorboard=False, main_prediction_enss=None):
        """Compatibility wrapper for older code that calls `validation_epoch_end`.

        Some training/eval scripts call `model.validation_epoch_end(predictions, ...)`
        passing the outputs from `trainer.predict`. The Lightning hook in this
        class is implemented as `on_validation_epoch_end`, which aggregates
        `self.validation_step_outputs`. To maintain backward compatibility we
        accept a `predictions` list, normalize/flatten it if needed, temporarily
        set `self.validation_step_outputs` to those predictions and delegate to
        `on_validation_epoch_end`.
        """
        # If no predictions provided, simply delegate to the existing hook
        if predictions is None:
            return self.on_validation_epoch_end(no_log_tensorboard=no_log_tensorboard,
                                                main_prediction_enss=main_prediction_enss)

        # Normalize predictions: trainer.predict may return a list of batches or
        # a list of lists; flatten one level if necessary.
        if isinstance(predictions, list) and len(predictions) > 0 and isinstance(predictions[0], list):
            flat_predictions = []
            for p in predictions:
                flat_predictions.extend(p)
        else:
            flat_predictions = list(predictions)

        # Preserve previous stored outputs (if any), set our temporary ones,
        # call the existing aggregation, then restore.
        prev_outputs = getattr(self, 'validation_step_outputs', None)
        self.validation_step_outputs = flat_predictions
        try:
            result = self.on_validation_epoch_end(no_log_tensorboard=no_log_tensorboard,
                                                 main_prediction_enss=main_prediction_enss)
        finally:
            # restore previous state to avoid side-effects
            self.validation_step_outputs = prev_outputs if prev_outputs is not None else []
